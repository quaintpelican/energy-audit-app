...
function blankAudit(){
  return {
    ...
    endUseModels:[],ecmPortfolios:[],weatherDatasets:[],manufacturerPerformanceDatasets:[],rcxContainers:[],qaFindingStates:[],qaDeclarations:{fieldScopeReviewed:false,analysisScopeReviewed:false},
...
function render(){
  ...
  renderAnalysisQueue();renderAuditQa();renderEndUseAnalysis();renderEndUseRollups();renderEndUseSources();renderPortfolios();renderFieldExitReview();
...
async function copyPrompt(){
  const prompt=`Act as a senior energy engineer performing an ASHRAE Level 2 analysis. Review the attached Audist V6.0 audit package. Read manifest.json first, treat audit.json as canonical, and use CSV/photo files as interoperable evidence. Review auditQa findings and engineer dispositions without treating deterministic QA as a substitute for professional judgment. Distinguish standalone ECM savings from adjusted portfolio savings. Respect confirmed interaction methods, sequence traces, excluded alternatives, utility/end-use boundaries, provenance, evidence, maturity, QA flags, stale status, and engineering readiness. Do not invent interaction factors, equipment specifications, measurements, schedules, utility rates, costs, savings, billing periods, end-use allocations, or weather effects.`;
...
showDashboard();
if("serviceWorker" in navigator){ navigator.serviceWorker.register("sw.js?v=6.0.0").catch(console.error); }
