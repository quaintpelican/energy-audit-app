...
## V6.3 ECM candidate registry

V6.3 distinguishes lightweight opportunity flags and advisory candidates from accepted ECMs. Master templates define applicability, field/analysis/recommended inputs, recipes, risks, and implementation considerations, but never deemed savings or generic percentages. Only explicit engineer acceptance creates a normal ECM; reject/defer state remains historical, and custom ECM creation remains available in Analysis Mode. Existing ECMs are unchanged.
